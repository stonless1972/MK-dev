...
def postAnalysis(masterFileName,hashcode):
    ...
    with open(masterFileName) as FO:
        ...
        for i in range(len(blocks)): ## each block
            ...
            matB_FN       = None
            ss_FN         = None
            for j in range(len(linesInBlock)):
                line    = linesInBlock[j]
                ind, fn = line.split()
                try:
    ...
    ##
    for i in range(len(dat_min_master)):
...
if __name__=='__main__':
    ...
    print('rhos:', rhos)
    ivpsc_hard = False
    if args.fnhrd_vpsc is not None:
        ivpsc_hard=True
    ...
    with open(rstFileName,'w') as rstFile:
        for i in range(len(results)): ## paths
            for j in range(len(results[i])): ## psi0s
                cmd = results[i][j].get()
